...
# For Splitter_DG_base_v1
def quad_term(c, S0, S1, n):
    return S0 - 2 * c * S1 + n * c ** 2

# For Splitter_DG_base_v1 and Splitter_DG_base_v2
cdef double compute_max_prop_balance_v1(
    int[:, :] count_L,
    int[:, :] count_R,
    double[:] total_count,
    int i,
...
# For Splitter_DG_fullopt and Splitter_DG_adafullopt
cdef double compute_max_prop_balance_v2(
    int[::1] count_L,
    int[::1] count_R,
    double[:] total_count,
) nogil:
    ...
    cpdef get_split(
        ...
        for feature in feature_indices:
            ...
            for i in range(N_i):
                ...
                split_idx = i + 1
                max_diff = compute_max_prop_balance_v1(
                    count_mat_L, count_mat_R, total_count, split_idx
                )
    ...
    cpdef get_split(
        ...
        for feature in feature_indices:
            ...
            for i in range(N_i):
                ...
                max_diff_p.value = compute_max_prop_balance_v2(
